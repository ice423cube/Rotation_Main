...
function addRotation(ismobiledevice, viewportWidth)
  {
	...
	if (ismobiledevice == true){
		var new_panel_width = viewportWidth;
		var new_filmstrip_style = 'none';
	} else {
		var new_panel_width = parseInt(Drupal.settings.rotationMain.rotation_main_width);
		var new_filmstrip_style = String(Drupal.settings.rotationMain.rotation_main_filmstrip_style);
	}
	...
	if(show_captions == 1){
		$('#rotation').galleryView({
			...
			filmstrip_style: new_filmstrip_style,
			show_captions: true,
...
Drupal.behaviors.rotationMain = {
 ...
 attach:function() {
			...
			$(document).ready(function(){
					...
					addRotation(ismobiledevice, viewportWidth);
					...
					var rounded;
					rounded = String(Drupal.settings.rotationMain.rotation_main_rounded_corners);
					
					if((rounded == "yes" && IEversion == 0) || (rounded == "yes" && IEversion >= 9)){
						$(".panel").corner("round 5px");
						$(".panel img").corner("round 5px");
